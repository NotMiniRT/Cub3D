#!/bin/bash

# Colors for better output
RED='\033[1;31m'
GREEN='\033[1;32m'
CYAN='\033[1;36m'
NC='\033[0m' # No Color

# Path to the executable and correct maps
EXECUTABLE="./cub3D"
CORRECT_MAPS_DIR="./assets/mandatory/maps"
TOTAL_MAPS=0
PASSED_MAPS=0

# Create logs directory if it doesn't exist
mkdir -p scripts/logs

# Timestamp for log file name
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="scripts/logs/correct_maps_${TIMESTAMP}.log"

# Start log file with header
echo "Correct Maps Tests - $(date)" > $LOG_FILE
echo "===============================" >> $LOG_FILE

echo -e "${CYAN}Starting correct maps tests...${NC}"

# Check if executable exists
if [ ! -f "$EXECUTABLE" ]; then
    echo -e "${RED}Error: Executable $EXECUTABLE not found. Please compile the project first.${NC}"
    echo "Error: Executable $EXECUTABLE not found. Please compile the project first." >> $LOG_FILE
    exit 1
fi

# Check if correct maps directory exists
if [ ! -d "$CORRECT_MAPS_DIR" ]; then
    echo -e "${RED}Error: Directory $CORRECT_MAPS_DIR not found.${NC}"
    echo "Error: Directory $CORRECT_MAPS_DIR not found." >> $LOG_FILE
    exit 1
fi

# Test all maps in the correct maps directory including hidden files
shopt -s dotglob  # Enable dotglob to include hidden files in glob patterns
for map_file in "$CORRECT_MAPS_DIR"/*; do
    # Skip directory entries . and ..
    if [[ "$(basename "$map_file")" == "." || "$(basename "$map_file")" == ".." ]]; then
        continue
    fi

    if [ -f "$map_file" ]; then
        MAP_NAME=$(basename "$map_file")
        TOTAL_MAPS=$((TOTAL_MAPS + 1))
        echo -e "${CYAN}Testing map: ${NC}$MAP_NAME"
        echo -e "\n[TEST] Testing map: $MAP_NAME" >> $LOG_FILE

        # Run the executable with the map and capture output
        OUTPUT=$("$EXECUTABLE" "$map_file" 2>&1 >/dev/null)
        EXIT_CODE=$?

        # Log the output and exit code
        echo "Output: $OUTPUT" >> $LOG_FILE
        echo "Exit code: $EXIT_CODE" >> $LOG_FILE

        # For correct maps, we expect NO error message
        if ! echo "$OUTPUT" | grep -q "Error"; then
            echo -e "${GREEN}✓ Test passed: Map loaded correctly${NC}"
            echo "PASS: Map loaded correctly" >> $LOG_FILE
            PASSED_MAPS=$((PASSED_MAPS + 1))
        else
            echo -e "${RED}✗ Test failed: Map produced an error${NC}"
            echo -e "${RED}Output was: ${NC}$OUTPUT"
            echo "FAIL: Map produced an error" >> $LOG_FILE
        fi
        echo "----------------------------------------"
        echo "----------------------------------------" >> $LOG_FILE
    fi
done
shopt -u dotglob  # Disable dotglob when done

# Print summary
echo -e "${CYAN}Test summary: ${PASSED_MAPS}/${TOTAL_MAPS} correct maps passed${NC}"
echo -e "\n===============================" >> $LOG_FILE
echo "Test summary: ${PASSED_MAPS}/${TOTAL_MAPS} correct maps passed" >> $LOG_FILE

# Exit with error if any test failed
if [ "$PASSED_MAPS" -ne "$TOTAL_MAPS" ]; then
    echo -e "${RED}Some maps failed to load correctly!${NC}"
    echo "Some maps failed to load correctly!" >> $LOG_FILE
    exit 1
else
    echo -e "${GREEN}All maps loaded successfully!${NC}"
    echo "All maps loaded successfully!" >> $LOG_FILE
    exit 0
fi
