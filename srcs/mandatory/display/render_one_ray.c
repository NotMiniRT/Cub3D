...
{
	return (((int *) \
		(img->buffer + (texture_y * img->line_bytes) + (texture_x)))[0]);
}

static inline void	change_pixel_color(t_image_cub *img, int clr, int x, int y)
{
...
{
	render_calc->height = \
		(WINDOW_HEIGHT / (render_calc->res[0] * main_struct->cos_R_H_tab[row]));
	...
	render_calc->texPos = \
			-(WINDOW_HEIGHT - render_calc->height) * 0.5 * render_calc->step;
	render_calc->height_check_minus = \
		(WINDOW_HEIGHT - render_calc->height) * 0.5;
	render_calc->height_check_plus = \
		(WINDOW_HEIGHT + render_calc->height) * 0.5;
