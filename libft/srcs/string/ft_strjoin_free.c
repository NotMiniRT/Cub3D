...
/*   Updated: 2025/06/09 17:11:42 by agantaum         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 *
 * This function joins two strings (s1 and s2) into a newly allocated string.
 * After the concatenation is complete, the memory allocated for the first
 * string (s1) is automatically freed.
 *
 * @param s1 The first string to be concatenated (will be freed)
 * @param s2 The second string to be concatenated (preserved)
 * @return A newly allocated string containing the concatenation of s1 and s2,
 *         or NULL if memory allocation fails or if either input is NULL
 *
 * @warning The first parameter (s1) will be freed inside this function,
 *          so it must be heap-allocated and should not be used after calling
 *          this function.
 */
char	*ft_strjoin_free2(char *s1, const char *s2)
{
	char	*res;
	size_t	s1_len;
	size_t	i;
